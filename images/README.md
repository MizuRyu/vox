...
- `settings.png`: 本体と同じ `SettingsView` を合成設定・架空のマイク情報・架空の辞書（2 行）でオフスクリーン描画したもの。
...
録音・入力監視・前面アプリの取得・スクリーンキャプチャは行わない。個人の設定ファイル・辞書・発話履歴を読み込まない。キーは変更例として cmd+opt+space を表示し、製品の既定値とは区別する。

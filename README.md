...
- **よく間違う語は辞書で直せる。** 「置き換える表記」と「入れたい表記」を 1 行ずつ書いたファイルを置くと、貼り付ける本文で置き換えます。
...
設定画面はメニューバーの「設定…」、HUD の歯車、`⌘,` のいずれかで開きます。録音とファイル検索のショートカット、自動Enter、使用するマイク、通話向けの音声処理（実験的）を変更でき、辞書ファイルもここから開けます。

マイクは「自動」が既定です。macOS の既定入力・出力がともに Bluetooth 系で出力が使用中なら、内蔵マイクがあれば録音に使います。「macOSの既定」や個別のマイクも選べます。macOS 全体の設定は変えません。各項目の意味は [使い方](docs/usage.md#設定) を参照してください。

...
診断ログへの発話本文は既定オフです。`--log-text` を明示した場合だけ確定本文が残ります。履歴保存は別機能として継続します。履歴・計測・検索フォルダのファイルは regular file・所有者・リンク数を検証して 0600 で開きます（`folders.json` は追記ではなく全文の書き直しです）。既存の共有親ディレクトリの権限は変更しません。不要な履歴・ログは本体を停止してから該当ファイルを削除できます。

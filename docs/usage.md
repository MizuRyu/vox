...
一度ファイル検索から確定したフォルダ（Git 管理下のもの、最大20件）は、次回の起動から索引をメモリに持ちます。そのフォルダでパレットを開くと候補がすぐに出て、裏で最新の状態に差し替わります。編集やコミットは自動で索引に反映します。持てる量には上限があり、超えた分は古いフォルダから外れて、開くたびの読み込みに戻ります。設定項目はありません。
